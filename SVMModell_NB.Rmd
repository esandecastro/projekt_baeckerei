...
# daten <- sample_frac(daten, .10)


# Bestimmung der Indizes des Traininsdatensatzes
train_ind <- sample(seq_len(nrow(daten)), size = floor(0.70 * nrow(daten)))
# Teilen in Trainings- und Testdatensatz
daten_train  <- daten[train_ind, ]
daten_test <- daten[-train_ind, ]
...
svm_tune <- tune (svm, Umsatz ~ Warengruppe +
...
for (j in 1:length(levels(daten$Warengruppe))) {
  ...
  pred_train <- predict(svm_tune$best.model, daten_train[daten_train$Warengruppe == levels(daten$Warengruppe)[j], ])
  pred_test <- predict(svm_tune$best.model, daten_test[daten_test$Warengruppe == levels(daten$Warengruppe)[j], ])
  
  print(mape(daten_train$Umsatz[daten_train$Warengruppe == levels(daten$Warengruppe)[j]], pred_train))
  print(mape(daten_test$Umsatz[daten_test$Warengruppe == levels(daten$Warengruppe)[j]], pred_test))
}
